---
title: GateForge · 能力发布控制面
aliases: [GateForge, gateforge, 朱雀, zhuque]
tags: [GateForge, 架构, 数字员工, MCP]
created: 2026-07-31
---

# GateForge · 能力发布控制面

GateForge 把企业的 REST API 池，编译成经过测试、审批、版本化、可回滚的**数字员工能力**，以声明式配置发布到 Nacos，由 Higress 网关执行。
...
## 二、三方分工：GateForge 不在调用链上

这是整个架构最关键的一条边界。GateForge 是**纯控制面**——只生产静态 JSON 配置和证据记录，不收发任何 MCP 协议消息。数字员工运行时，GateForge 就算整个宕掉，线上服务也不受影响。

| | GateForge | Nacos | Higress |
|---|---|---|---|
| **角色** | 控制面 | 配置面 | 数据面 |
...
flowchart TB
    ...
    ADMIN["管理员"]
    ZQ["GateForge<br/>control plane"]
    PUB["发布执行器<br/>Nacos 快照 · 失败恢复"]
    NACOS["Nacos<br/>配置面"]
    ...
    ZQ --> PUB
    PUB -->|"写原生 MCP Registry<br/>nacos_payload"| NACOS
    NACOS -->|"Nacos3 source 自动发现"| HIGRESS

    AGENT -->|"MCP over SSE / JSON-RPC<br/>运行平台配置鉴权"| HIGRESS
...
> [!important] 读这张图的关键
> 最下面那条链路 `数字员工 → Higress → 企业后端` 里**没有 GateForge**。
> 它写完配置就退场，不持有连接、不转发消息、不在故障域内。
>
> Higress 的服务来源、路由、鉴权和流量策略由运行平台独立维护；GateForge 每次发布不调用网关 Console。
...
| ⑥ 审批与发布 | 审批签内容哈希；将冻结快照发布到 Nacos，失败恢复原快照 | **必须人点** |
...
| `release → deploy_record` | 1 : N | 新发布记录 `nacos` 结果；历史数据可能保留旧的 `higress_auth` 记录 |

---

## 六、发布到 Nacos，Higress 自动发现

GateForge 的事务边界止于 Nacos：发布前保存 Registry 快照，写入后等待 MCP detail
及其 endpoint 完整可读；失败则恢复原快照。Higress 的 Nacos3 source、路由与鉴权
由运行平台手工配置一次，之后 watcher 自动跟随 Registry 状态。
...
sequenceDiagram
    ...
    participant H as Higress watcher

    U->>P: 点击发布
    P->>P: ① 前置校验<br/>Nacos AI MCP Admin API
    P->>N: ② 读取当前 MCP 快照

    alt 正常路径
        P->>N: ③ 写原生 MCP service 定义
        N-->>P: detail + endpoint 可读
        P->>P: ④ 落 nacos deploy_record
        N-->>H: watcher 发现变更
        H->>H: 自动生成数据面路由
        P-->>U: 返回 MCP URL
    else Nacos 写入失败
        ...
        N-->>P: 失败：命名空间不存在
        P->>N: ③' 恢复原 MCP 快照
        P-->>U: 发布失败：已恢复<br/>并指向具体修复位置
    end
```

> [!important] 网关边界
> GateForge 不判断或修改 Higress 鉴权。生产环境的默认拒绝、消费者凭据、限流和
> 审计由 Higress 自身部署流程负责，不能与每次 MCP 内容发布绑成同一个代码事务。
...
> 5. **发布只管理 Nacos。** 失败恢复 Registry 快照；Higress 由运行平台独立维护。
> 6. **命名生成即固化。** `mcp-{部门}-{员工}` 落库后不可变更，对账与回滚全靠它。
> 7. **控制面不写网关。** 发布、回滚、漂移和生命周期不得调用 Higress Console 写接口。
...
| 阶段 | 状态 | 内容 |
|---|---|---|
| **P1 · 前端** | ✅ 已完成 | 设计系统、控制面页面与真实 API 接入；新建空数据库首次启动显示空库，不展示演示数据。 |
| **P2 · 后端** | ✅ 已接入 | Java 17 + Spring Boot 3，M1~M10 控制面实现、PostgreSQL 迁移、回收站与审计记录已接入。 |
| **P3 · 联调** | ✅ 主链路已通 | 已用真实 OpenAPI fixture 跑通 GateForge 发布 → Nacos Registry → Higress 自动路由 → MCP tools/call → REST 上游；写/删接口仍需受控 fixture、清理与回读断言。 |
...
| `m6_testing` | 三层测试 | `L0StaticChecker`、`L1ContractTester`（真实测试/预发上游）、`L2AgentEvaluator` |
| `m7_gate` | 门禁 | `GateRule` + `GateEngine` |
| `m8_deploy` | Nacos MCP 发布 | `NacosTarget`、`DualTargetPublisher`、`DeployPrecheck`；Higress 由运行平台独立维护 |
