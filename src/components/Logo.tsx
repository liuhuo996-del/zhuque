 ...
 *
 * - 拱门：执行前的策略门禁（Gate）
 * - 熔芯：将 API 和工具铸成可发布能力（Forge）
 * - 印记：通过门禁后的可审计发布
 *
 * brand 是默认的品牌版；inverse 用于无彩色的深色场景。
 * ink / vermilion 保留了旧组件的调用兼容性。
 */
export function LogoMark({ size = 28, variant = 'brand', className }: LogoMarkProps) {
  ...
  return (
    ...
    >
      ...
      {/* Forge: 一颗向上的熔芯，轮廓简化以保证 24px 尺寸下仍可辨识。 */}
      <path
      ...
      {/* Release seal: 将“通过 / 发布”收进标志，避免另外叠加状态色。 */}
